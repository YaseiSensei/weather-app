...
export default function Dashboard() {
    ...
    return (
        <Segment className={`ui  ${theme} `} style={{ paddingLeft: "200px", paddingRight: "200px", marginTop: "-1px", paddingTop: "110px", height: "630px", borderRadius: "0px" }}>
            <Grid   >
                <Grid.Row columns={7}>
                    {loding && weather.data.map((x, i) =>
                        <Grid.Column className={x.datetime === currentDate ? "teal" : ""} key={i} style={{ paddingRight: "5px", paddingLeft: "5px", borderRadius: "5px" }}>
                            <Card color='teal'> <Card.Content extra>{gunler[new Date((x.datetime)).getDay()]}  </Card.Content> <Image style={{ backgroundColor: "" }} src={`https://www.weatherbit.io/static/img/icons/${x.weather.icon}.png`} wrapped ui={true} />
